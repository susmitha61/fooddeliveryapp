...
import {
  Star, Store, Search, Filter, Trash2, EyeOff, Eye, RefreshCw,
  MessageSquare, AlertTriangle, CheckCircle2, ShoppingBag, Utensils, Shield, Edit2, X, Save, Sparkles
} from 'lucide-react';
import {
  getReviewDishName,
  getCleanReviewComment,
} from '@/lib/utils';
...
export default function AdminReviewsPage() {
  ...
  const { data: restaurantsRes, isLoading: restaurantsLoading } = useGetRestaurantsQuery(
    { page: 0, size: 100 },
  ...
  const filteredReviews = reviews.filter((rev) => {
    ...
    if (visibilityFilter === 'VISIBLE' && !isVis) return false;
    if (visibilityFilter === 'HIDDEN' && isVis) return false;
  ...
  const handleHide = async (rev: any) => {
    const reviewId = rev.reviewId || rev.id;
    try {
      await hideReview(reviewId).unwrap();
      toast.success('Review hidden on server');
      refetchReviews();
      refetchSummary();
    } catch (err: any) {
      toast.error(err?.data?.message || 'Failed to hide review');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Monitor customer feedback, audit food & restaurant ratings, and moderate reviews."
        actions={
          <div className="flex items-center gap-2">
            <button
              onClick={() => { refetchReviews(); refetchSummary(); }}
              className="p-2.5 bg-[var(--surface-color)] border border-[var(--border-color)] hover:border-rose-500 rounded-xl text-xs font-bold transition-all"
      ...
      />
            ...
            <div className="bg-[var(--surface-color)] border border-[var(--border-color)] p-4 rounded-2xl flex flex-col md:flex-row gap-4 items-center justify-between">
              <div className="flex items-center gap-3 w-full md:w-auto">
                ...
                <div className="w-full md:w-72">
                  ...
                  <select
                    ...
                    className="w-full p-2.5 bg-[var(--background-color)] border border-[var(--border-color)] rounded-xl text-xs font-bold focus:outline-none focus:border-rose-500"
                  >
                    {restaurants.map((r) => (
              ...
              <div className="flex flex-wrap items-center gap-2 w-full md:w-auto justify-end">
                <div className="relative max-w-xs w-full sm:w-64">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 bg-[var(--background-color)] border border-[var(--border-color)] rounded-xl text-xs font-semibold focus:outline-none focus:border-rose-500"
                  />
                </div>
              ...
              <div className="flex items-center gap-2 pt-3 border-t border-[var(--border-color)]/60 overflow-x-auto pb-1 scrollbar-none">
                <button
                  ...
                  {filteredReviews.map((rev) => {
                    ...
                    return (
                      <div
                        ...
                        className={`bg-[var(--surface-color)] border p-5 rounded-2xl flex flex-col justify-between space-y-4 shadow-sm transition-all ${
                          isVis
                        ...
                        <div className="space-y-2.5">
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-full bg-rose-500/10 border border-rose-500/30 text-rose-400 font-bold flex items-center justify-center shrink-0">
                              ...
                              <div>
                                ...
                                <p className="text-[11px] text-[var(--text-muted)] font-mono">
                                  User ID: #{String(rev.userId || '').slice(-6)}
                                </p>
                              </div>
                            </div>
                          ...
                          <div className="flex flex-wrap items-center gap-2 pt-1">
                            ...
                            {isVis ? (
                              <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 flex items-center gap-1">
                                <Eye className="w-3 h-3" /> Visible to Stores
                              </span>
                            ) : (
                              <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-rose-500/20 text-rose-400 border border-rose-500/40 flex items-center gap-1">
                          ...
                          <p className="text-xs text-[var(--text-secondary)] leading-relaxed bg-[var(--background-color)]/60 p-3 rounded-xl border border-[var(--border-color)]/60 italic">
                            &quot;{getCleanReviewComment(rev.comment) || 'No comment provided.'}&quot;
                          </p>
                        </div>
                        ...
                        <div className="pt-2 border-t border-[var(--border-color)]/60 flex items-center justify-between gap-2">
                          {isVis ? (
                            <button
                              ...
                              disabled={isHiding}
                              className="px-3 py-1.5 rounded-xl border border-amber-500/30 bg-amber-500/10 hover:bg-amber-500/20 text-xs font-bold text-amber-400 transition-all flex items-center gap-1.5"
                              title="Hide this review from Store Managers and Restaurant Owners"
                            >
                            ...
                            <span
                              className="px-3 py-1.5 rounded-xl border border-rose-500/30 bg-rose-500/10 text-xs font-bold text-rose-400 flex items-center gap-1.5"
                              title="This review is hidden on the server"
                            >
                              <EyeOff className="w-3.5 h-3.5" />
                              <span>Hidden on Server</span>
                            </span>
                          )}
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="px-3 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/20 hover:bg-amber-500/20 text-amber-400 text-xs font-bold transition-all flex items-center gap-1.5"
                            >
                            ...
                            <button
                              ...
                              disabled={isDeleting}
                              className="px-3 py-1.5 rounded-xl bg-rose-500/10 border border-rose-500/20 hover:bg-rose-500/20 text-rose-400 text-xs font-bold transition-all flex items-center gap-1.5"
                            >
        ...
        {editingReview && (
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-[var(--surface-color)] border border-[var(--border-color)] rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-outfit font-extrabold text-lg">Edit Review</h3>
                <button
              ...
              <form onSubmit={handleSaveEdit} className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full p-3 bg-[var(--background-color)] border border-[var(--border-color)] rounded-xl text-xs font-medium focus:outline-none focus:border-rose-500"
                  />
                </div>
                ...
                <div className="flex items-center justify-end gap-2 pt-2">
                  ...
                  <button
                    type="submit"
                    disabled={isUpdating}
                    className="px-4 py-2 bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold rounded-xl flex items-center gap-1.5 shadow-sm disabled:opacity-50"
                  >
